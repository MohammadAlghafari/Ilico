...
export 'package:google_sign_in_platform_interface/google_sign_in_platform_interface.dart' show SignInOption;
...
class GoogleSignInAccount implements GoogleIdentity {
  ...
  Future<GoogleSignInAuthentication> get authentication async {
    ...
    final GoogleSignInTokenData response = await GoogleSignInPlatform.instance.getTokens(
      email: email,
  ...
  @override
  int get hashCode => Object.hash(displayName, email, id, photoUrl, idToken, serverAuthCode);
...
class GoogleSignIn {
  ...
  Stream<GoogleSignInAccount?> get onCurrentUserChanged => _currentUserController.stream;
  ...
  Future<GoogleSignInAccount?> _callMethod(Function method) async {
    ...
    return _setCurrentUser(
        response != null && response is GoogleSignInUserData ? GoogleSignInAccount._(this, response) : null);
  }
  ...
  Future<void> _ensureInitialized() {
    return _initialization ??= GoogleSignInPlatform.instance.initWithParams(SignInInitParameters(
  ...
  Future<GoogleSignInAccount?> signInSilently({
    ...
    try {
      return await _addMethodCall(GoogleSignInPlatform.instance.signInSilently, canSkipCall: !reAuthenticate);
    } catch (_) {
      if (suppressErrors) {
        return null;
      } else {
  ...
  Future<GoogleSignInAccount?> signIn() {
    ...
    bool isCanceled(dynamic error) => error is PlatformException && error.code == kSignInCanceledError;
